...
public class Bird : MonoBehaviour {
    ...
    void Update()
    {
        ...
        Vector3 sep = Separate(flock.GetBirds());
        Vector3 ali = Align(flock.GetBirds());
        Vector3 coh = Cohesion(flock.GetBirds());
        ...
        if (pos.x > 80) pos.x = -80; //-80
        if (pos.x < -80) pos.x = 80; //80
        if (pos.y > 50) pos.y =  50; //-50
        if (pos.y < 50) pos.y = 50;  //50
    ...
    Vector3 Separate(List<GameObject> birds)
    {
    ...
    Vector3 Align(List<GameObject> birds)
    {
        ...
        foreach (GameObject other in birds)
        {
            ...
            if ((d > 0) && (d < neighborDistance))
            {
                ...
                sum += other.GetComponent<Bird>().velocity;
                count++;
            }
        }
    ...
    Vector3 Cohesion(List<GameObject> birds)
    {
